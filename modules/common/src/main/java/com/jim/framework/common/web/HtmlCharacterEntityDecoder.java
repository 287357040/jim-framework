package com.jim.framework.common.web;

/**
 * Helper for decoding HTML Strings by replacing character entity references
 * with the referred character.
 * 
...
class HtmlCharacterEntityDecoder {
	...
	private void findNextPotentialReference(int startPosition) {
		...
		do {
			...
			boolean isPotentialReference = nextPotentialReferencePosition != -1
	...
	private boolean processNumberedReference() {
		boolean isHexNumberedReference = originalMessage
				.charAt(nextPotentialReferencePosition + 2) == 'x'
				|| originalMessage.charAt(nextPotentialReferencePosition + 2) == 'X';
		try {
			int value = (!isHexNumberedReference) ? Integer.parseInt(
					getReferenceSubstring(2)) : Integer.parseInt(getReferenceSubstring(3), 16);
	...
	private String getReferenceSubstring(int referenceOffset) {
		return originalMessage.substring(nextPotentialReferencePosition
				+ referenceOffset, nextSemicolonPosition);
	}
